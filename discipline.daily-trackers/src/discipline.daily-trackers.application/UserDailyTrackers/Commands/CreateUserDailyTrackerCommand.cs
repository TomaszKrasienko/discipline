...
public sealed record CreateUserDailyTrackerCommand(
    DailyTrackerId Id,
    AccountId AccountId,
    DateOnly Day) : ICommand;
    
public sealed class CreateUserDailyTrackerCommandHandler(
    IUserDailyTrackerFactory userDailyTrackerFactory,
    IReadWriteUserDailyTrackerRepository userDailyTrackerRepository) : ICommandHandler<CreateUserDailyTrackerCommand>
{
    public async Task HandleAsync(CreateUserDailyTrackerCommand command, CancellationToken cancellationToken)
    {
