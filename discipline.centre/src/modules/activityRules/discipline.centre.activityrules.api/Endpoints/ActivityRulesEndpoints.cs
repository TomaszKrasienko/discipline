    ...
    internal static WebApplication MapActivityRulesEndpoints(this WebApplication app)
    {
        app.MapPost(
                    ...
                    IValidator<ActivityRuleRequestDto> validator) => 
            {
                ...
                await dispatcher.HandleAsync(command.ToCreateCommand(userId.Value, activityRuleId), cancellationToken);
        ...
        app.MapPut(
                $"api/{ActivityRulesTag}/{{activityRuleId:ulid}}",
                async (
                    Ulid activityRuleId,
                    ActivityRuleRequestDto dto,
                    IValidator<ActivityRuleRequestDto> validator,
                    CancellationToken cancellationToken,
                    ICqrsDispatcher dispatcher,
                    IIdentityContext identityContext) =>
        {
            ...
            await dispatcher.HandleAsync(dto.ToUpdateCommand(account.Value, stronglyActivityRuleId), cancellationToken);
        ...
        app.MapDelete($"api/{ActivityRulesTag}/{{activityRuleId:ulid}}/stages/{{stageId:ulid}}", async (Ulid activityRuleId, 
            ...
            .Produces(StatusCodes.Status401Unauthorized, typeof(void))
            .Produces(StatusCodes.Status403Forbidden, typeof(void))
            .WithName("DeleteActivityRuleStage")
            .WithTags(ActivityRulesTag)
            .WithOpenApi(operation => new (operation)
            {
